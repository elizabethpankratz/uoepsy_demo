...
## By default, R will keep numeric-looking data numeric
...
### Associations between gender and stats anxiety
...
`r fontawesome::fa("hourglass-start", margin_right = '0.2em', height = '1em')` **For one minute:** Think to yourself about the questions.

`r fontawesome::fa("hourglass-half", margin_right = '0.2em', height = '1em')` **For one minute:** Ask your neighbour what they think. What's their reasoning? What's yours?
...
anx_fit2_latent_normals |>
  ...
  geom_vline(xintercept = anx_fit2$zeta, linetype = 'dotted', linewidth = 2) +
  geom_vline(xintercept = 0, colour = palette_gender[1]) +
  geom_vline(xintercept = mm_mean, colour = palette_gender[2]) +
  geom_vline(xintercept = ag_mean, colour = palette_gender[3]) +
  geom_text(data = rating_labs, aes(label = labs), family = 'Fira Sans', size = 12) +
  scale_fill_manual(values = palette_gender) +
  theme_classic() +
  theme(
    axis.line.x.top = element_blank(),
    text = element_text(family = 'Fira Sans', size = 24),
    axis.text.x.top = element_text(family = 'Fira Sans', size = 24)
  ) +
  scale_y_continuous(NULL, breaks = NULL, limits = c(0, 0.5)) +
  scale_x_continuous(
    'Anxiety (imaginary underlying variable)', 
  ...
  geom_segment(aes(x= 0, y = 0.42, xend = mm_mean, yend = 0.42), linewidth = 1, arrow = arrow(length = unit(0.2,"cm")), colour = palette_gender[2]) +
  geom_label(aes(x = -0.15, y = 0.44), label = round(mm_mean, 2), fill = 'white', colour = palette_gender[2], family = 'Fira Sans', size = 6) +
  geom_segment(aes(x= 0, y = 0.46, xend = ag_mean, yend = 0.46), linewidth = 1, arrow = arrow(length = unit(0.2,"cm")), colour = palette_gender[3]) +
  geom_label(aes(x = 0.2, y = 0.48), label = round(ag_mean, 2), fill = 'white', colour = palette_gender[3], family = 'Fira Sans', size = 6) +
  NULL
```
]
...
But it's common practice to compare these *t*-values to a standard normal distribution (treating them like *z* scores).
